...
These are not commited to the repo, but you can download them using the ETL if you want real data (see below).
...
The app is typically available at `http://localhost:5173`. The API is served on `http://localhost:8080`, and the local Postgres instance is on `localhost:5432` with the database credentials configured in the compose stack.
